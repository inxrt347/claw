...
class WorldObject {
  ...
  // ВНЕСЕННЫЙ МЕТОД: проверка столкновения
  doOverlap(a, b) {
    return b.x > a.x && b.x < a.x + a.w && b.y > a.y && b.y < a.y + a.h
  }

  // ВНЕСЕННЫЙ МЕТОД: поиск ближайшей игрушки
  getClosestToy() {
    const claw = {
      y: this.y + maxArmLength + machineBuffer.y + 7,
      x: this.x + 7,
      w: 40,
      h: 32,
    }
    const overlappedToys = elements.toys.filter(t => {
      return this.doOverlap(t, claw)
    })
